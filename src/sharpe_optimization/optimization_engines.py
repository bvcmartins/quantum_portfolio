...
def genetic_algorithm_sharpe(data, population_size=500, num_generations=1000, mutation_rate=0.05, elitism=0.1, min_investment=0.001, max_weight=None):
    """
    ...
    - Max weight per asset: adaptive diversification constraint
    ...
    Args:
        ...
        max_weight: Maximum weight per asset (default None = adaptive: min(0.10, 10 * equal_weight))
...
def scipy_minimize_sharpe(data, min_investment=0.001, max_weight=None):
    """
    ...
    # Bounds: minimum allocation per asset, adaptive max per asset for diversification
...
def riskfolio_sharpe_optimized(data, min_investment=0.001, max_weight=None):
    """
...
def dwave_nl_sharpe(data, budget, min_investment):
    ...
    # Adaptive max weight: 10x equal weight or 10% cap, whichever is lower
    equal_weight = 1.0 / n_cols
    max_weight_per_asset = min(0.10, 10 * equal_weight)
    print(f"Adaptive max weight per asset: {max_weight_per_asset*100:.2f}%")
    ...
    investments = model.integer(n_cols, lower_bound=min_investment_dollars, upper_bound=max_investment_dollars)
    ...
    # Maximum diversification constraint: adaptive max weight per asset
    max_allowed_investment = model.constant(int(budget * max_weight_per_asset))
    model.add_constraint((investments <= max_allowed_investment).all())
    constraint_count = n_cols  # One constraint per asset
    print(f"  - Added {n_cols} max diversification constraints (max {max_weight_per_asset*100:.1f}% per asset, adaptive)")

    # Calculate weights from investments
    print("Constructing objective function...")
    #weights = [safe_divide(investments[i], total_investment) for i in range(n_cols)]
    weights = safe_divide(investments, total_investment)
    ...
    constraint_count += 1  # Add 1 for min std constraint
    ...
    print(f"  - Total constraints: {2 + constraint_count} (2 budget + {n_cols} max diversification + 1 min std)")
